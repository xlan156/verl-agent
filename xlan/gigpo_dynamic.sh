...
#SBATCH --time=4:00:00
#SBATCH --output=job_log/GiGPO-%j/output.txt
#SBATCH --error=job_log/GiGPO-%j/error.txt
...
export EXPERIMENT_NAME="gigpo-n${MAX_CHEMICAL_N}-nodynamic-teacher0.0-${MODEL_NAME}-0902"
export EPOCHS=50
export MAX_STEP=35
...
export TEACHER_REWARD_COEF=0.0
...
for eval_n in 1 2 3 4; do
    ...
    OUTPUT_PATH="xlan/results/${EXPERIMENT_NAME}-eval-n${eval_n}-${SLURM_JOB_ID:-local}.json" \
