...
def _fallback_candidate_action(
    ...
    candidate_action = _infer_move_or_rotate(text)
    if candidate_action is not None:
        return candidate_action

    action_name = _infer_object_action(text)
    if action_name in _SINGLE_OBJECT_ACTIONS:
        uuid = _find_uuid_from_text(text, object_seen)
        return {"action": action_name, "arg1": uuid}

    if action_name in _DOUBLE_OBJECT_ACTIONS:
        uuid2 = _find_uuid_from_text("jar", object_seen)
        uuid1 = _find_uuid_from_text(re.sub(r"jar", " ", text.lower()), object_seen)
        if uuid1 and uuid2:
